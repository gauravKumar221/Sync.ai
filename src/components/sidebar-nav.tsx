        ...
        <SidebarMenu>
          {navItems.map((item) => (
            <SidebarMenuItem key={item.title}>
              <Link href={item.href}>
                <SidebarMenuButton isActive={isActive(item.href)} size="lg" tooltip={item.title}>
                  <item.icon className="h-5 w-5" />
                  <span className={cn(isCollapsed && !isMobile && "hidden")}>{item.title}</span>
                </SidebarMenuButton>
              </Link>
            </SidebarMenuItem>
          ))}
      ...
      <SidebarFooter>
        <SidebarMenu>
          <SidebarMenuItem>
            <Link href={settingsNav.href}>
              <SidebarMenuButton isActive={isActive(settingsNav.href)} size="lg" tooltip={settingsNav.title}>
                <settingsNav.icon className="h-5 w-5" />
                <span className={cn(isCollapsed && !isMobile && "hidden")}>{settingsNav.title}</span>
              </SidebarMenuButton>
            </Link>
          </SidebarMenuItem>
        </SidebarMenu>
        <div className="flex items-center justify-between p-2">
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <div className={cn("flex flex-col text-sm transition-opacity duration-200", isCollapsed && !isMobile ? 'opacity-0' : 'opacity-100' )}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className={cn("h-7 w-7", isCollapsed && !isMobile ? 'opacity-0 pointer-events-none' : '')}>
                <MoreHorizontal />
              </Button>
            </DropdownMenuTrigger>
